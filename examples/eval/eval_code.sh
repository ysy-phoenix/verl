...
# python -m sglang_router.launch_server --model-path $HOME/checkpoints/code-r1-13k-leetcode2k-taco-grpo/global_step_2048/actor/huggingface/ --dp 4
...
evalhub run --model Qwen2.5-7B-Instruct --tasks humaneval --output-dir $HOME/metrics/$exp_name/global_step_$step/ -p temperature=$temperature
evalplus.evaluate --dataset humaneval --samples $HOME/metrics/$exp_name/global_step_$step/humaneval.jsonl

evalhub run --model Qwen2.5-7B-Instruct --tasks mbpp --output-dir $HOME/metrics/$exp_name/global_step_$step/ -p temperature=$temperature
evalplus.evaluate --dataset mbpp --samples $HOME/metrics/$exp_name/global_step_$step/mbpp.jsonl

evalhub run --model Qwen2.5-7B-Instruct --tasks livecodebench --output-dir $HOME/metrics/$exp_name/global_step_$step/ -p temperature=$temperature
